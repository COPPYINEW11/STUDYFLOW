...
- **Primary (Deep Indigo / Navy Blue):** Used for the core structural elements, active states in navigation, and primary "deep work" actions (`#24389c`).
- **Secondary (Soft Mint / Teal):** Reserved for positive reinforcement, completion states, and progress bars. It signifies "growth" and "success" (`#006b5c`).
- **Accent (Vibrant Orange):** High-visibility color used exclusively for temporal urgency—timers, upcoming deadlines, and critical warnings (`#ff9800`).
- **Neutral (Deep Charcoal):** Provides the foundation for typography and dark-mode surfaces (`#1c1b1b`).
